...
if __name__ == "__main__":
    ...
    real_names = list(glob.glob('{}/*_gt.png'.format(args.path)))
    fake_names = list(glob.glob('{}/*_rlt.png'.format(args.path)))
    ...
    for rname, fname in zip(real_names, fake_names):
        idx += 1
        ridx = rname.rsplit("_gt")[0].split('_')[1]
        fidx = rname.rsplit("_rlt")[0].split('_')[1]
